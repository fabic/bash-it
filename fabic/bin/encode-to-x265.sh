...
#                -print0)
    ...
    ( while read -r _perms _hlinks _user _group _size _date _time fil ; do
          ...
          ext="mp4"
          tmpext=".tmp.$ext"
          outfile="$outdir/${filebn}.H265.CRF$CRF.${ext}"
          outfile_tmp="$outdir/${filebn}${tmpext}"

          if [ -s "$outfile" ]; then
            echo
            echo "(!!) SKIPPING EXISTING OUTPUT FILE: $outfile"
            echo
            continue
          fi
          ...
          if true; then
            echo "Generating a thumbnail image..."
            if ! ffmpeg -ss 10 -i "$fil" \
              -v quiet -hide_banner \
              -vf "thumbnail,scale=-1:720" \
              -frames:v 1                  \
              -vsync vfr                   \
              -y "$thumbnail_image_file" </dev/null ;
              xdg-open "$thumbnail_image_file" &
            then
          ...
          if true; then
              ffmpeg_cmd=(
                ffmpeg
                  ...
                  -crf $CRF -maxrate 800k -bufsize 2M
                  -bf 2 -flags +cgop -g 25
...
EOF
          ...
          echo -e "\n\n\t( NEXT ITERATION )\n\n"
               ...
               # See https://en.wikipedia.org/wiki/Video_file_format
                    ...
                    -iregex ".*\.\(xvid\|divx\|webm\|mkv\|flv\|vob\|ogv\|ogg\|drc\|mng\|avi\|MTS\|M2TS\|mov\|qt\|wmv\|yuv\|dv\|rm\|rmvb\|asf\|amv\|mp4\|m4p\|m4v\|mpg\|mp2\|mpeg\|mpe\|mpv\|mpg\|mpeg\|m2v\|m4v\|svi\|3gp\|3g2\|mxf\|roq\|nsv\|flv\|f4v\|f4p\|f4a\|f4b\)$" \
